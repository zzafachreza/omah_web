...
class Swasta extends CI_Controller{
  ...
  function update(){
    ...
    $id = $this->input->post('id_'.$this->modul());
        ...
        $sql="UPDATE data_".$this->modul()." SET ";
            ...
            $jml =$this->db->query("SHOW FULL COLUMNS from `data_".$this->modul()."` WHERE FIELD !='id_".$this->modul()."'")->num_rows();
          ...
          $no=1;
           ...
           foreach($this->db->query("SHOW FULL COLUMNS from `data_".$this->modul()."` WHERE FIELD !='id_".$this->modul()."'")->result() as $col){
              ...
              if($col->Type=="varchar(444)"){
                  ...
                  $foto_old = $_POST[$col->Field.'_old'];


                if(!empty($_FILES[$col->Field]['name'])){
        
                    $myFoto = $this->uploadFoto($col->Field,date('ymdhis'));
                    unlink($foto_old);
